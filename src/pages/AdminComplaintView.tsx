...
export default function AdminComplaintView() {
  ...
  const fetchData = async () => {
    ...
    const { data: c } = await supabase
      ...
      .single();
    if (c) {
      setComplaint(c as unknown as ComplaintData);
      setNewStatus(c.status);
    }

    const { data: r } = await supabase
      .from("responses")
      .select("id, response_text, created_at, admin_id")
      .eq("complaint_id", id)
    ...
    if (r && r.length > 0) {
      ...
      setResponses(
        r.map((x) => ({
          ...x,
          admin_name: profileMap.get(x.admin_id) || "Admin",
        }))
      );
    } else {
      setResponses([]);
  ...
  const handleSubmitResponse = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!responseText.trim() || !user || !complaint) return;
    setSubmitting(true);
    ...
    if (newStatus !== complaint.status) {
      await supabase
        .from("complaints")
        .update({ status: newStatus as typeof STATUSES[number] })
        .eq("id", complaint.id);
    }
  ...
  if (!complaint) {
    return (
      <div className="min-h-screen bg-secondary">
        <AppNavbar />
        <div className="flex items-center justify-center py-20 text-muted-foreground">Complaint not found</div>
      </div>
    );
  }
        ...
        <div className="bg-card rounded-xl shadow-sm border border-border p-6 sm:p-8 animate-slide-up">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-foreground text-sm leading-relaxed whitespace-pre-wrap">{complaint.description}</p>
            </div>
            {complaint.image_url && (
              <div>
